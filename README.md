...
HID 输出支持三种基础模式：短按（按下后约 45ms 松开）、长按（当前约 1.5 秒）和组合键（按顺序按下所有键后统一松开）。每次发送会加入很小的随机时序浮动：短按 ±12ms、长按 ±90ms、组合键逐键间隔 ±8ms。默认映射集中在 `src/sc_pad_keymap.cpp`：起落架 `N`、舱门 `/`、采矿 `M`、VTOL `K`、灯光 `L`、量子长按 `B`、扫描 `V`、星图 `F2`、电源 `U`、引擎 `I`、ATC `Left Alt + N`、离席长按 `Y`。SHIP 页的护盾控制使用数字小键盘 `Num 2/4/5/6/7/8/9`，电源分配使用 `F5–F8`。
